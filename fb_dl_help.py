...
bot_config = {
    "api_id": "20891704",
    "api_hash": "4d9707a98e36730bb0002c9820099f79",
    "owner_user_id": 7567233794,
    "secret_code": "814814",
    "bot_channel": "@Dev_CodeNexis",
    "reports_channel": 2389059947}

API_ID = bot_config['api_id']
API_HASH = bot_config['api_hash']
BOT_TOKEN = '7082450044:AAHJ9D1xbWdHzosaZKnmvQnKGrzEi0E39Ws'

robot = bot.TelegramClient("Dev", API_ID, API_HASH).start(bot_token=BOT_TOKEN)

if os.path.exists('./db') == False:
    os.mkdir('./db')

# Database setup
conn_users = sqlite3.connect('./db/bot_clients_details_test.db')
...
# details of channels details
cursor.execute(
    """CREATE TABLE IF NOT EXISTS channels (channel_id INTEGER PRIMARY KEY, owner_user_id INTEGER NOT NULL)""")
conn_users.commit()
...
async def facebook(event):
    ...
    async with robot.conversation(event.sender_id) as conv:
        ...
        try:
            ...
            if youtube_url.startswith('http') and ('facebook.com' in youtube_url):
                video_info = await loop.run_in_executor(executor, fb_dl_help.fetch_youtube_video_info,
                                                        youtube_url)
                ...
                if "error" in video_info:
                    ...
                    response_text = (
                        ...
                        f"**Channel URL:** [View FaceBook Channel]({video_info['channel_url']})\n"
                    ...
                    video_id = await asyncio.create_task(
                        fb_dl_help.select_format(event, robot, video_info['formats'], 'mp4', 'vid'))

                    # Download Video
                    yt_dl_respond = await loop.run_in_executor(executor, fb_dl_help.yt_dl, user_id, youtube_url,
                                                            video_id)
...
async def quick_mode_facebok(event, message, user_id):
    ...
    try:
        ...
        get_redirect_link = await asyncio.to_thread(facebook.resolve_redirect, message)

        if get_redirect_link == 1:
            await robot.send_message(user_id, f"Error resolving URL")
            return

        video_info = await asyncio.to_thread(facebook.fetch_facebook_video_info, get_redirect_link)
        result_insta = await asyncio.to_thread(facebook.download_facebook_content, get_redirect_link, output_dir)
        if result_insta == 0:
...
async def message_filter(event, message: str, user_id):
    ...
    await event.reply(f"💬 You said: {domain} 🗣️")
    return
